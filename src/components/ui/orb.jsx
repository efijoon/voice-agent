...
function Scene({
  ...
  const offsets = useMemo(() =>
    new Float32Array(Array.from({ length: 7 }, () => random() * Math.PI * 2)), [random])
  ...
  useFrame((_, delta) => {
    ...
    curInRef.current += (targetIn - curInRef.current) * 0.25
    curOutRef.current += (targetOut - curOutRef.current) * 0.25
